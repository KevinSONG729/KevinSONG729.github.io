...
function playVids(videoId) {
    ...
    if (vid.readyState > 3) {
        ...
        function trackLocationTouch(e) {
            ...
            if(flag == true){
                if(Math.abs(tmp - pos_array[0])<= 0.03 && Math.abs(tmp - pos_array[1])<= 0.03) pos_array[0] = tmp;
                else if(Math.abs(tmp - pos_array[0])<= 0.03) pos_array[0] = tmp;
                else if(Math.abs(tmp - pos_array[1])<= 0.03) pos_array[1] = tmp;
            }
        }
        function setflag(e){
            console.log(e);
