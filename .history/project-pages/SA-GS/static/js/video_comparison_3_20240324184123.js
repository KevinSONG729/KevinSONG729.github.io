...
function playVids(videoId) {
    ...
    if (vid.readyState > 3) {
        ...
        function trackLocationTouch(e) {
            ...
            if(flag == true){
                if(Math.abs(tmp - pos_array[0])<= 0.01 && Math.abs(tmp - pos_array[1])<= 0.01) pos_array[0] = tmp;
                else if(Math.abs(tmp - pos_array[0])<= 0.01) pos_array[0] = tmp;
                else if(Math.abs(tmp - pos_array[1])<= 0.01) pos_array[1] = tmp;
            }
        }
        function setflag(e){
            console.log(e);
