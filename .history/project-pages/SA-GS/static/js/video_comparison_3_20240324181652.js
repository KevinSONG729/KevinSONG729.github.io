...
function playVids(videoId) {
    ...
    if (vid.readyState > 3) {
        ...
        function trackLocationTouch(e) {
            // Normalize to [0, 1]
            bcr = videoMerge.getBoundingClientRect();
            tmp = ((e.touches[0].pageX - bcr.x) / bcr.width)
            if(Math.abs(tmp - position)<= 0.005 && Math.abs(tmp - position2)<= 0.005) position = tmp;
            else if(Math.abs(tmp - position)<= 0.005) position = tmp;
            else if(Math.abs(tmp - position2)<= 0.005) position2 = tmp;
        }
        ...
        function drawLoop() {
            ...
            colStart = (vidWidth * position2).clamp(0.0, vidWidth);
            colWidth = (vidWidth - (vidWidth * position2)).clamp(0.0, vidWidth);
            mergeContext.drawImage(vid, colStart+vidWidth, 0, colWidth, vidHeight, colStart, 0, colWidth, vidHeight);
            ...
            currX_array[0] = vidWidth * position;
            currX_array[1] = vidWidth * position2;
            var position_array = new
